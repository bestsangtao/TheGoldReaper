...
//====================================================================
// Doi tuong CTrade dung chung cho toan bo OrderSend/OrderModify/OrderClose/OrderDelete.
// SetAsyncMode(false): giu dong bo 100% (doi server tra loi that truoc khi return),
// giong hanh vi OrderSend() tho truoc day - KHONG doi logic, chi doi lop thuc thi.
// LogLevel(LOG_LEVEL_NO): tat log rieng cua CTrade de tranh in trung voi log kieu
// MT4 tu viet (MT4PrintTradeOk/MT4PrintTradeReject) o duoi.
CTrade trade;
               ...
               datetime expiration=0,color arrow_color=clrNONE)
{
   ...
   {
      // Lenh thi truong: dung filling MT4SelectFilling() tu dong do theo broker,
      // gia lay truc tiep tu Ask/Bid hien tai - giong het logic ban goc.
      trade.SetTypeFilling(MT4SelectFilling(symbol));
      otype=(cmd==OP_BUY)?ORDER_TYPE_BUY:ORDER_TYPE_SELL;
      execPrice=(cmd==OP_BUY)?SymbolInfoDouble(symbol,SYMBOL_ASK):SymbolInfoDouble(symbol,SYMBOL_BID);
      ok=trade.PositionOpen(symbol,otype,volume,execPrice,stoploss,takeprofit,comment);
   }
   else
   {
      // Lenh cho: giu nguyen filling RETURN co dinh nhu ban goc (khong dung
      // MT4SelectFilling o nhanh nay - dung y voi request.type_filling=ORDER_FILLING_RETURN cu).
      switch(cmd)
      {
         case OP_BUYLIMIT:  otype=ORDER_TYPE_BUY_LIMIT;  break;
         case OP_SELLLIMIT: otype=ORDER_TYPE_SELL_LIMIT; break;
         case OP_BUYSTOP:   otype=ORDER_TYPE_BUY_STOP;   break;
         case OP_SELLSTOP:  otype=ORDER_TYPE_SELL_STOP;  break;
      }
      trade.SetTypeFilling(ORDER_FILLING_RETURN);
      ENUM_ORDER_TYPE_TIME ttime=(expiration>0)?ORDER_TIME_SPECIFIED:ORDER_TIME_GTC;
      ok=trade.OrderOpen(symbol,otype,volume,0.0,price,stoploss,takeprofit,ttime,expiration,comment);
   }

   uint retcode=trade.ResultRetcode();
   if(!ok && retcode==0) retcode=TRADE_RETCODE_ERROR;
   ...
   if(ok && (retcode==TRADE_RETCODE_DONE || retcode==TRADE_RETCODE_DONE_PARTIAL || retcode==TRADE_RETCODE_PLACED))
   {
      ...
      // Dung execPrice (gia du dinh luc gui lenh), KHONG dung trade.ResultPrice():
      // voi lenh cho (pending), result.price = 0 vi lenh chua khop, se log sai "at 0.00000".
      PrintFormat("%s #%I64d %s %.2f %s at %.5f sl: %.5f tp: %.5f ok",
                  "open",g_mt4_lastTicket,MT4OrderTypeName((int)otype),volume,
                  symbol,execPrice,stoploss,takeprofit);
      return g_mt4_lastTicket;
   }
   PrintFormat("failed %s %s %.2f %s at %.5f sl: %.5f tp: %.5f [%s] (retcode=%u)",
               "open",MT4OrderTypeName((int)otype),volume,symbol,execPrice,
...
bool OrderModify(long ticket,double price,double stoploss,double takeprofit,datetime expiration,color arrow_color=clrNONE)
{
   ...
   {
      // Vi the dang mo -> chi sua SL/TP (khong co "price" cho vi the).
      symbol=PositionGetString(POSITION_SYMBOL);
      ok=trade.PositionModify((ulong)ticket,stoploss,takeprofit);
   }
   else if(::OrderSelect((ulong)ticket))
   {
      // Lenh cho -> sua ca gia/SL/TP/expiration.
      symbol=::OrderGetString(ORDER_SYMBOL);
      ENUM_ORDER_TYPE_TIME ttime=(expiration>0)?ORDER_TIME_SPECIFIED:ORDER_TIME_GTC;
      ok=trade.OrderModify((ulong)ticket,price,stoploss,takeprofit,ttime,expiration,0.0);
   }
   else
   {
...
bool OrderClose(long ticket,double lots,double price,int slippage,color arrow_color=clrNONE)
{
   ...
   bool ok;
   if(closeLots>=volume)
      ok=trade.PositionClose((ulong)ticket,deviation);
   else
      ok=trade.PositionClosePartial((ulong)ticket,closeLots,deviation);
   ...
   if(ok && (retcode==TRADE_RETCODE_DONE || retcode==TRADE_RETCODE_DONE_PARTIAL))
   {
      ...
      PrintFormat("close #%I64d %s %.2f %s at %.5f ok",
                  ...
                  closeLots,symbol,trade.ResultPrice());
      return true;
   }
   PrintFormat("failed close %s %.2f %s [%s] (retcode=%u, ticket=%I64d)",
               (posType==POSITION_TYPE_BUY)?"buy":"sell",closeLots,symbol,
...
bool OrderDelete(long ticket,color arrow_color=clrNONE)
{
   ...
   bool ok=trade.OrderDelete((ulong)ticket);
   ...
   PrintFormat("failed delete %s %.2f %s at %.5f [%s] (retcode=%u, ticket=%I64d)",
               delName,delVol,delSym,delPrice,trade.ResultRetcodeDescription(),retcode,ticket);
   return false;
}
...
// XAUUSD ton tai doc lap). MT4 mac dinh luon la hedging nen ban goc khong
