...
#property description "- Fixed WorldTimeServer GMT"
#property description "- Fixed OnlyUp"
#property description "- Automatic NFP calendar with retry"
#property description "- Optional Friday close"
#property description "- Highest Balance panel"
#property description "- Virtual/Server Stop Orders panel"
#property description "- Async Virtual entry with retry"
#property description "- Learned latency and 5-tick confirmation"
#property description "- Missing URL warning"
#property description "- MT4-style trade logging"
#property description "- Added optional inputs"
...
//====================================================================
// ===== VIRTUAL STOP ORDERS (pending orders are kept client-side) =====
// When UseVirtualStopOrders=true, every Buy Stop or Sell Stop requested
// through the MQL4-style OrderSend() wrapper is NOT sent to the broker as
// a real pending order. It is stored in EA memory and represented by a
// horizontal price line on the chart. On every tick, the EA checks the
// current market price and immediately sends a real market order when the
// virtual trigger price is reached.
//
// The other MT4 compatibility functions (OrderSelect, OrderModify,
// OrderDelete and MT4OrdersTotal) also include virtual orders, so the
// original strategy can manage them exactly like normal pending orders
// without changing the trading logic elsewhere in the EA.
//====================================================================
struct VirtualOrder
{
   long     ticket;
   string   symbol;
   int      type;        // OP_BUYSTOP or OP_SELLSTOP
   double   lots;
   double   price;        // virtual trigger price
   double   sl;
   double   tp;
   datetime openTime;
   datetime expiration;
   ...
   // Fast virtual-trigger execution state. These fields are used only when
   // UseVirtualStopOrders=true; normal/non-virtual OrderSend logic is untouched.
   bool     triggered;       // price has touched the virtual stop; never un-trigger it
   bool     sendPending;     // an asynchronous market request is awaiting server reply
   uint     requestId;       // MQL5 async request ID used to match OnTradeTransaction
   ulong    requestStartUs;  // local timestamp when the current async request was submitted
   ulong    nextRetryMs;     // earliest GetTickCount64() value for the next attempt
   int      retryCount;
};
VirtualOrder g_vOrders[];
int          g_vCount=0;
long         g_vTicketSeq=1900000000; // separate ticket range to avoid broker-ticket collisions

// Recent tick-motion and measured execution-latency state used by
// AdaptiveVirtualEarlyTrigger. Five completed broker ticks are retained per
// symbol. Early activation is allowed only after at least three of those ticks
// move toward Entry and the latest tick is still moving in that direction.
#define VIRTUAL_TICK_HISTORY 5

struct VirtualTickMotion
{
   string symbol;
   long   lastTimeMsc;
   double lastBid;
   ...
   // Ring buffer of signed quote changes. Positive Ask delta means upward
   // movement; negative Bid delta means downward movement.
   double askDelta[VIRTUAL_TICK_HISTORY];
   double bidDelta[VIRTUAL_TICK_HISTORY];
   double tickIntervalMs[VIRTUAL_TICK_HISTORY];
   int    historyPos;       // next ring-buffer slot to overwrite
   int    historyCount;     // 0..VIRTUAL_TICK_HISTORY

   double avgTickIntervalMs;

   // Learned from OrderSendAsync -> TRADE_TRANSACTION_REQUEST completion.
   // Until a successful sample exists, TERMINAL_PING_LAST is used as fallback.
   double avgServerLatencyMs;
   int    latencySamples;
};
VirtualTickMotion g_vTickMotion[];
int               g_vTickMotionCount=0;

int VirtualTickMotionIndex(string symbol)
{
   for(int i=0;i<g_vTickMotionCount;i++)
      if(g_vTickMotion[i].symbol==symbol) return i;

   int idx=g_vTickMotionCount++;
   ArrayResize(g_vTickMotion,g_vTickMotionCount);
   g_vTickMotion[idx].symbol=symbol;
   g_vTickMotion[idx].lastTimeMsc=0;
   g_vTickMotion[idx].lastBid=0.0;
   g_vTickMotion[idx].lastAsk=0.0;
   g_vTickMotion[idx].historyPos=0;
   g_vTickMotion[idx].historyCount=0;
   g_vTickMotion[idx].avgTickIntervalMs=0.0;
   g_vTickMotion[idx].avgServerLatencyMs=0.0;
   g_vTickMotion[idx].latencySamples=0;
   for(int i=0;i<VIRTUAL_TICK_HISTORY;i++)
   {
      g_vTickMotion[idx].askDelta[i]=0.0;
      g_vTickMotion[idx].bidDelta[i]=0.0;
      g_vTickMotion[idx].tickIntervalMs[i]=0.0;
   }
   return idx;
}

// Store exactly one sample for each genuine broker tick. OnTimer calls carrying
// the same tick.time_msc cannot create duplicate history samples.
void VirtualTickMotionUpdate(string symbol,const MqlTick &tick)
{
   int idx=VirtualTickMotionIndex(symbol);
   if(g_vTickMotion[idx].lastTimeMsc<=0)
   {
      g_vTickMotion[idx].lastTimeMsc=tick.time_msc;
      g_vTickMotion[idx].lastBid=tick.bid;
      g_vTickMotion[idx].lastAsk=tick.ask;
      return;
   }

   if(tick.time_msc<=g_vTickMotion[idx].lastTimeMsc) return;

   long dt=tick.time_msc-g_vTickMotion[idx].lastTimeMsc;
   if(dt<=0) return;

   int pos=g_vTickMotion[idx].historyPos;
   g_vTickMotion[idx].askDelta[pos]=tick.ask-g_vTickMotion[idx].lastAsk;
   g_vTickMotion[idx].bidDelta[pos]=tick.bid-g_vTickMotion[idx].lastBid;
   g_vTickMotion[idx].tickIntervalMs[pos]=(double)dt;
   g_vTickMotion[idx].historyPos=(pos+1)%VIRTUAL_TICK_HISTORY;
   if(g_vTickMotion[idx].historyCount<VIRTUAL_TICK_HISTORY)
      g_vTickMotion[idx].historyCount++;

   if(g_vTickMotion[idx].avgTickIntervalMs<=0.0)
      g_vTickMotion[idx].avgTickIntervalMs=(double)dt;
   else
      g_vTickMotion[idx].avgTickIntervalMs=0.75*g_vTickMotion[idx].avgTickIntervalMs+0.25*(double)dt;
...
void VirtualTickMotionFeedSymbol(string symbol)
{
   MqlTick tick;
   ZeroMemory(tick);
   if(SymbolInfoTick(symbol,tick) && tick.bid>0.0 && tick.ask>0.0)
      VirtualTickMotionUpdate(symbol,tick);
}

// Learn the complete successful request-response time, including terminal,
// network and broker processing. Extreme values are ignored so reconnects or a
// frozen terminal do not permanently distort the prediction.
void VirtualObserveServerLatency(string symbol,double latencyMs)
{
   if(latencyMs<1.0 || latencyMs>5000.0) return;

   int idx=VirtualTickMotionIndex(symbol);
   if(g_vTickMotion[idx].latencySamples<=0)
      g_vTickMotion[idx].avgServerLatencyMs=latencyMs;
   else
      g_vTickMotion[idx].avgServerLatencyMs=
         0.75*g_vTickMotion[idx].avgServerLatencyMs+0.25*latencyMs;

   if(g_vTickMotion[idx].latencySamples<1000000)
      g_vTickMotion[idx].latencySamples++;
}

double VirtualEstimatedServerLatencyMs(int idx)
{
   if(idx<0 || idx>=g_vTickMotionCount) return 0.0;

   double latency=g_vTickMotion[idx].avgServerLatencyMs;
   if(latency<=0.0)
   {
      // TERMINAL_PING_LAST is reported in microseconds by MT5.
      long pingUs=TerminalInfoInteger(TERMINAL_PING_LAST);
      if(pingUs>0) latency=(double)pingUs/1000.0;
   }

   // Last fallback before the EA has any execution or ping sample: one normal
   // observed tick interval. Keep the prediction bounded against stale data.
   if(latency<=0.0) latency=g_vTickMotion[idx].avgTickIntervalMs;
   return MathMax(5.0,MathMin(2000.0,latency));
}

double VirtualAdaptiveEarlyDistance(string symbol,const MqlTick &tick,int orderType)
{
   if(!AdaptiveVirtualEarlyTrigger) return 0.0;
   ...
   double pipSize=((digits==3 || digits==5)?10.0:1.0)*point;
   int count=g_vTickMotion[idx].historyCount;

   // No early trigger until three completed tick movements are available.
   if(count<3) return 0.0;

   int directionalTicks=0;
   double directionalMoveSum=0.0;
   double netDirectionalMove=0.0;
   double weightedVelocitySum=0.0;
   double weightSum=0.0;
   double latestDirectionalMove=0.0;

   for(int age=0;age<count;age++)
   {
      int pos=(g_vTickMotion[idx].historyPos-1-age+VIRTUAL_TICK_HISTORY)%VIRTUAL_TICK_HISTORY;
      double dt=MathMax(1.0,g_vTickMotion[idx].tickIntervalMs[pos]);
      double move=(orderType==OP_BUYSTOP)
                  ?g_vTickMotion[idx].askDelta[pos]
                  :-g_vTickMotion[idx].bidDelta[pos];

      if(age==0) latestDirectionalMove=move;
      netDirectionalMove+=move;

      // Recent ticks carry more weight: 5,4,3,2,1 for a full history.
      // Opposite ticks are included with a negative sign so choppy movement
      // cannot look faster than the real net motion toward Entry.
      double weight=(double)(count-age);
      weightedVelocitySum+=(move/dt)*weight;
      weightSum+=weight;

      if(move>0.0)
      {
         directionalTicks++;
         directionalMoveSum+=move;
      }
   }

   // A single spike is not enough. At least three of the last 3-5 ticks must
   // move toward Entry, the latest tick must agree, and the net move must still
   // be toward Entry. Otherwise the order waits for its exact stored Entry.
   if(directionalTicks<3 || latestDirectionalMove<=0.0 || netDirectionalMove<=0.0)
      return 0.0;

   double weightedVelocity=(weightSum>0.0)?weightedVelocitySum/weightSum:0.0;
   double averageDirectionalJump=directionalMoveSum/(double)directionalTicks;
   double latencyMs=VirtualEstimatedServerLatencyMs(idx);
   double movementDuringLatency=weightedVelocity*latencyMs;

   // Use the larger of the latest confirmed jump, the average directional jump
   // and expected movement during the learned server delay.
   double estimated=MathMax(latestDirectionalMove,
                            MathMax(averageDirectionalJump,movementDuringLatency));

   // Safety cap only; no fixed minimum is imposed. For 2-digit Gold, 100 pips
   // equals 1.00 in price. Entry, SL and TP remain unchanged.
   double maxDistance=100.0*pipSize;
   return NormalizeDouble(MathMin(maxDistance,estimated),digits);
}

int VFindIndexByTicket(long ticket)
{
   for(int i=0;i<g_vCount;i++) if(g_vOrders[i].ticket==ticket) return i;
   return -1;
}

int VFindIndexByRequestId(uint requestId)
{
   if(requestId==0) return -1;
   for(int i=0;i<g_vCount;i++)
      if(g_vOrders[i].sendPending && g_vOrders[i].requestId==requestId) return i;
   return -1;
}

string VLineName(long ticket)  { return "vstop_line_"+IntegerToString((int)ticket,0,32); }
string VLabelName(long ticket) { return "vstop_lbl_"+IntegerToString((int)ticket,0,32); }

// Keep each Virtual Buy/Sell Stop caption attached to the right edge of the
// chart. The horizontal position is measured from CORNER_RIGHT_UPPER, so it
// automatically follows every chart-width change and is not affected by
// Chart Shift. The vertical position is recalculated from the virtual price.
#define VIRTUAL_LABEL_RIGHT_GAP_PX  6

void VUpdateLabelPosition(int idx)
{
   if(idx<0 || idx>=g_vCount) return;

   string lb=VLabelName(g_vOrders[idx].ticket);
   if(ObjectFind(0,lb)<0) return;
   ...
   int middleOffset=visibleBars/2;
   if(middleOffset<1) middleOffset=1;
   int middleShift=firstVisible-middleOffset;
   if(middleShift<0) middleShift=0;
   datetime visibleTime=iTime(_Symbol,_Period,middleShift);
   if(visibleTime<=0) visibleTime=TimeCurrent();

   int x=0;
   int y=0;
   if(!ChartTimePriceToXY(0,0,visibleTime,g_vOrders[idx].price,x,y)) return;

   int chartHeight=(int)ChartGetInteger(0,CHART_HEIGHT_IN_PIXELS,0);
   if(y<10) y=10;
   if(chartHeight>0 && y>chartHeight-2) y=chartHeight-2;

   ObjectSetInteger(0,lb,OBJPROP_CORNER,CORNER_RIGHT_UPPER);
   ObjectSetInteger(0,lb,OBJPROP_ANCHOR,ANCHOR_RIGHT_LOWER);
   ObjectSetInteger(0,lb,OBJPROP_XDISTANCE,VIRTUAL_LABEL_RIGHT_GAP_PX);
   ObjectSetInteger(0,lb,OBJPROP_YDISTANCE,y);
}

void VUpdateAllLabelPositions()
{
   for(int i=0;i<g_vCount;i++)
      VUpdateLabelPosition(i);
}

void VDrawLine(int idx)
{
   if(idx<0 || idx>=g_vCount) return;
   string nm=VLineName(g_vOrders[idx].ticket);
   color  clr=(g_vOrders[idx].type==OP_BUYSTOP)?VirtualBuyLineColor:VirtualSellLineColor;
   if(ObjectFind(0,nm)<0) ObjectCreate(0,nm,OBJ_HLINE,0,0,g_vOrders[idx].price);
   ObjectSetDouble(0,nm,OBJPROP_PRICE,g_vOrders[idx].price);
   ObjectSetInteger(0,nm,OBJPROP_COLOR,clr);
   ObjectSetInteger(0,nm,OBJPROP_STYLE,STYLE_DASH);
   ObjectSetInteger(0,nm,OBJPROP_WIDTH,1);
   ObjectSetInteger(0,nm,OBJPROP_BACK,true); // keep the virtual line behind the main information panel
   ObjectSetInteger(0,nm,OBJPROP_SELECTABLE,false);
   ObjectSetInteger(0,nm,OBJPROP_HIDDEN,true);

   string lb=VLabelName(g_vOrders[idx].ticket);
   int dig=(int)SymbolInfoInteger(g_vOrders[idx].symbol,SYMBOL_DIGITS);
   string txt=StringFormat("VIRTUAL %s %.2f @ %s",
                (g_vOrders[idx].type==OP_BUYSTOP)?"BUY STOP":"SELL STOP",
                g_vOrders[idx].lots,
                DoubleToString(g_vOrders[idx].price,dig));
   // OBJ_LABEL uses pixel coordinates. Recreate an older OBJ_TEXT label when
   // updating from a previous version so its right-edge anchoring is reliable.
   if(ObjectFind(0,lb)>=0 &&
      (ENUM_OBJECT)ObjectGetInteger(0,lb,OBJPROP_TYPE)!=OBJ_LABEL)
      ObjectDelete(0,lb);
   if(ObjectFind(0,lb)<0) ObjectCreate(0,lb,OBJ_LABEL,0,0,0);
   ObjectSetString(0,lb,OBJPROP_TEXT,txt);
   ObjectSetInteger(0,lb,OBJPROP_COLOR,clr);
   ObjectSetInteger(0,lb,OBJPROP_FONTSIZE,8);
   ObjectSetInteger(0,lb,OBJPROP_CORNER,CORNER_RIGHT_UPPER);
   ObjectSetInteger(0,lb,OBJPROP_ANCHOR,ANCHOR_RIGHT_LOWER); // caption stays above the virtual line
   ObjectSetInteger(0,lb,OBJPROP_BACK,true); // keep the virtual label behind the main information panel
   ObjectSetInteger(0,lb,OBJPROP_SELECTABLE,false);
   ObjectSetInteger(0,lb,OBJPROP_HIDDEN,true);
   VUpdateLabelPosition(idx);
}

void VRemoveLine(long ticket)
{
   ObjectDelete(0,VLineName(ticket));
   ObjectDelete(0,VLabelName(ticket));
}

long VirtualOrderAdd(string symbol,int cmd,double lots,double price,double sl,double tp,
                      string comment,int magic,datetime expiration)
{
   int idx=g_vCount;
   ArrayResize(g_vOrders,g_vCount+1);
   g_vTicketSeq++;
   g_vOrders[idx].ticket=g_vTicketSeq;
   g_vOrders[idx].symbol=symbol;
   g_vOrders[idx].type=cmd;
   g_vOrders[idx].lots=lots;
   g_vOrders[idx].price=price;
   g_vOrders[idx].sl=sl;
   g_vOrders[idx].tp=tp;
   g_vOrders[idx].openTime=TimeCurrent();
   g_vOrders[idx].expiration=expiration;
   g_vOrders[idx].comment=comment;
   g_vOrders[idx].magic=magic;
   g_vOrders[idx].triggered=false;
   g_vOrders[idx].sendPending=false;
   g_vOrders[idx].requestId=0;
   g_vOrders[idx].requestStartUs=0;
   g_vOrders[idx].nextRetryMs=0;
   g_vOrders[idx].retryCount=0;
   g_vCount++;
   VDrawLine(idx);
   VirtualPanelUpdate();
   PrintFormat("[VIRTUAL] virtual stop order #%I64d created: %s %.2f %s @ %s (not sent to broker)",
               g_vOrders[idx].ticket,(cmd==OP_BUYSTOP)?"buy stop":"sell stop",
               lots,symbol,DoubleToString(price,(int)SymbolInfoInteger(symbol,SYMBOL_DIGITS)));
   return g_vOrders[idx].ticket;
}

void VirtualOrderRemoveAt(int idx)
{
   if(idx<0 || idx>=g_vCount) return;
   VRemoveLine(g_vOrders[idx].ticket);
   for(int i=idx;i<g_vCount-1;i++) g_vOrders[i]=g_vOrders[i+1];
   g_vCount--;
   ArrayResize(g_vOrders,g_vCount);
   VirtualPanelUpdate();
}

bool VirtualOrderDeleteByTicket(long ticket)
{
   int idx=VFindIndexByTicket(ticket);
   if(idx<0) return false;
   VirtualOrderRemoveAt(idx);
   return true;
}

bool VirtualOrderModifyByTicket(long ticket,double price,double sl,double tp,datetime expiration)
{
   int idx=VFindIndexByTicket(ticket);
   if(idx<0) return false;
   g_vOrders[idx].price=price;
...
// Number of row objects currently rendered by the Virtual panel.
// Used only for cleaning up rows when the number of virtual orders decreases.
int g_vPanelRowsRendered=0;

// Lightweight row used by the auxiliary panel when Virtual mode is OFF.
// In that mode, pending Buy Stop/Sell Stop orders are read directly from
// the trade server and displayed with the same columns as virtual orders.
struct ServerPanelStopOrder
{
   ulong  ticket;
   string symbol;
   int    type;
...
int ServerPanelCollectStopOrders(ServerPanelStopOrder &rows[])
{
   ArrayResize(rows,0);
   int count=0;
   ...
   for(int i=0;i<total;i++)
   {
      ...
      // The panel belongs to this chart, so do not mix pending orders from
      // other symbols into it.
      if(symbol!=_Symbol) continue;

      ArrayResize(rows,count+1);
      rows[count].ticket=ticket;
      rows[count].symbol=symbol;
      rows[count].type=(orderType==ORDER_TYPE_BUY_STOP)?OP_BUYSTOP:OP_SELLSTOP;
      rows[count].lots=OrderGetDouble(ORDER_VOLUME_CURRENT);
      rows[count].price=OrderGetDouble(ORDER_PRICE_OPEN);
      rows[count].sl=OrderGetDouble(ORDER_SL);
      rows[count].tp=OrderGetDouble(ORDER_TP);
      count++;
   }
...
// Remove every object used by the Virtual Stop Orders panel.
void VirtualPanelDeleteObjects()
{
   ObjectDelete(0,"vpnl_bg");
   ObjectDelete(0,"vpnl_title");
   ObjectDelete(0,"vpnl_summary");
   ObjectDelete(0,"vpnl_separator");
   ObjectDelete(0,"vpnl_columns");
   ObjectDelete(0,"vpnl_empty");
   ObjectDelete(0,"vpnl_more");

   // Remove the six independently positioned column headers.
   for(int c=0;c<6;c++)
      ObjectDelete(0,"vpnl_head"+IntegerToString(c,0,32));

   // Remove the five vertical separators between the six columns.
   for(int c=0;c<5;c++)
      ObjectDelete(0,"vpnl_vline"+IntegerToString(c,0,32));

   // Also remove object names used by the older panel implementation.
   ObjectDelete(0,"vpnl_hdr");
   int rowsToDelete=MathMax(g_vPanelRowsRendered,g_vCount);
   for(int r=0;r<rowsToDelete;r++)
   {
      ObjectDelete(0,"vpnl_row"+IntegerToString(r,0,32));
      ObjectDelete(0,"vpnl_mark"+IntegerToString(r,0,32));
      for(int c=0;c<6;c++)
         ObjectDelete(0,"vpnl_cell"+IntegerToString(r,0,32)+"_"+IntegerToString(c,0,32));
   }
   g_vPanelRowsRendered=0;
}

void VirtualPanelSetLabel(string name,int x,int y,string text,color clr,int fontSize,
...
// Draw a one-pixel vertical separator between adjacent panel columns.
void VirtualPanelSetVerticalLine(string name,int x,int y,int height,color clr)
{
   ...
   ObjectSetInteger(0,name,OBJPROP_YSIZE,MathMax(height,1));
...
// Draw a compact panel matching the size, colors and spacing of the original
// information panel. It is placed to the right of the main panel whenever
// possible; on narrow charts it automatically moves below the main panel.
void VirtualPanelUpdate()
{
   // Keep the auxiliary panel visible independently of Virtual mode.
   // It is hidden only when the main information panel or this panel itself
   // is disabled in the inputs.
   if(!ShowInfoPanel || !ShowVirtualOrdersPanel)
   {
      VirtualPanelDeleteObjects();
      return;
   }

   ServerPanelStopOrder serverRows[];
   int serverCount=0;
   if(!UseVirtualStopOrders)
      serverCount=ServerPanelCollectStopOrders(serverRows);

   int shown=UseVirtualStopOrders?g_vCount:serverCount;

   double scale=MathMax(InfoPanelSizeAdjust,0.50);

   // Read the main panel's actual on-chart geometry. This avoids the one-pixel
   // offset caused by using rounded values here while the main panel uses
   // truncated integer sizes. Fall back to the original constants before the
   // main panel object has been created.
   int mainX=5;
   int mainY=20;
   int mainW=(int)(350.0*InfoPanelSizeAdjust);
   int mainH=(int)(366.0*InfoPanelSizeAdjust);
   if(总_17_bo_8C)
      mainH+=(int)((总_378_in_5D80+3)*总_362_do_5CC8);

   if(ObjectFind(0,"infopanel_rectangle")>=0)
   {
      mainX=(int)ObjectGetInteger(0,"infopanel_rectangle",OBJPROP_XDISTANCE);
      mainY=(int)ObjectGetInteger(0,"infopanel_rectangle",OBJPROP_YDISTANCE);
      mainW=(int)ObjectGetInteger(0,"infopanel_rectangle",OBJPROP_XSIZE);
      mainH=(int)ObjectGetInteger(0,"infopanel_rectangle",OBJPROP_YSIZE);
   }

   int gap=MathMax(8,(int)MathRound(10.0*scale));
   int panelW=mainW;
   int padding=MathMax(6,(int)MathRound(8.0*scale));
   int titleH=MathMax(18,(int)MathRound(22.0*scale));
   int rowH=MathMax(15,(int)MathRound(18.0*scale));
   int panelH=(shown>0)
              ? titleH+rowH*2+shown*rowH+padding*2
              : titleH+rowH*3+padding*2;

   // Always keep the auxiliary panel fixed on the left side, directly
   // below the main information panel.
   int panelX=mainX;
   int panelY=mainY+mainH+gap;

   int fontSize=(SetFontSize>0)?SetFontSize:总_372_in_5CFC;
   fontSize=MathMax(fontSize,7);
   color textColor=(color)总_329_ui_3104;
   color bgColor=(color)总_364_ui_5CD4;
   color borderColor=(color)0xFF0000;

   if(ObjectFind(0,"vpnl_bg")<0) ObjectCreate(0,"vpnl_bg",OBJ_RECTANGLE_LABEL,0,0,0);
   ObjectSetInteger(0,"vpnl_bg",OBJPROP_CORNER,CORNER_LEFT_UPPER);
   ObjectSetInteger(0,"vpnl_bg",OBJPROP_XDISTANCE,panelX);
   ObjectSetInteger(0,"vpnl_bg",OBJPROP_YDISTANCE,panelY);
   ObjectSetInteger(0,"vpnl_bg",OBJPROP_XSIZE,panelW);
   ObjectSetInteger(0,"vpnl_bg",OBJPROP_YSIZE,panelH);
   ObjectSetInteger(0,"vpnl_bg",OBJPROP_BGCOLOR,bgColor);
   ObjectSetInteger(0,"vpnl_bg",OBJPROP_COLOR,borderColor);
   ObjectSetInteger(0,"vpnl_bg",OBJPROP_BORDER_COLOR,borderColor);
   ObjectSetInteger(0,"vpnl_bg",OBJPROP_BORDER_TYPE,BORDER_FLAT);
   ObjectSetInteger(0,"vpnl_bg",OBJPROP_STYLE,STYLE_SOLID);
   ObjectSetInteger(0,"vpnl_bg",OBJPROP_WIDTH,2);
   ObjectSetInteger(0,"vpnl_bg",OBJPROP_BACK,false);
   ObjectSetInteger(0,"vpnl_bg",OBJPROP_SELECTABLE,false);
   ObjectSetInteger(0,"vpnl_bg",OBJPROP_HIDDEN,true);
   ObjectSetInteger(0,"vpnl_bg",OBJPROP_ZORDER,10);

   int textX=panelX+padding;
   int y=panelY+padding;
   string panelTitle=UseVirtualStopOrders?"VIRTUAL STOP ORDERS":"SERVER STOP ORDERS";
   VirtualPanelSetLabel("vpnl_title",textX,y,panelTitle,textColor,fontSize+1,"Arial");
   VirtualPanelSetLabel("vpnl_summary",panelX+panelW-(int)MathRound(105.0*scale),y+1,
                        StringFormat("ACTIVE: %d",shown),textColor,fontSize,"Arial");

   y+=titleH;
   VirtualPanelSetLabel("vpnl_separator",textX,y,
                        "------------------------------------------------------",textColor,fontSize,"Arial");
   y+=rowH;

   // Position every column independently so spacing stays balanced at every
   // panel scale. Type receives slightly more room; the five numeric columns
   // are distributed across the remaining width.
   int rowTextX=textX+MathMax(6,(int)MathRound(8.0*scale));
   int contentRight=panelX+panelW-padding;
   int contentW=MathMax(1,contentRight-rowTextX);
   int colX[6];
   colX[0]=rowTextX;
   colX[1]=rowTextX+(int)MathRound(contentW*0.19);
   colX[2]=rowTextX+(int)MathRound(contentW*0.32);
   colX[3]=rowTextX+(int)MathRound(contentW*0.49);
   colX[4]=rowTextX+(int)MathRound(contentW*0.65);
   colX[5]=rowTextX+(int)MathRound(contentW*0.81);

   // Type stays left-aligned. Every numeric column uses the centre of its
   // allocated area, so the SL/TP headings sit exactly above their values.
   int colCenter[6];
   colCenter[0]=colX[0];
   colCenter[1]=(colX[1]+colX[2])/2;
   colCenter[2]=(colX[2]+colX[3])/2;
   colCenter[3]=(colX[3]+colX[4])/2;
   colCenter[4]=(colX[4]+colX[5])/2;
   colCenter[5]=(colX[5]+contentRight)/2;
   ...
   if(shown<=0)
   {
      ...
      string emptyText=UseVirtualStopOrders
                       ?"No active virtual stop orders"
                       :"No active server stop orders";
      VirtualPanelSetLabel("vpnl_empty",textX,y+rowH/2,
                           emptyText,textColor,fontSize,"Arial");
      ObjectDelete(0,"vpnl_more");
      for(int r=0;r<g_vPanelRowsRendered;r++)
      {
         ObjectDelete(0,"vpnl_row"+IntegerToString(r,0,32));
         ObjectDelete(0,"vpnl_mark"+IntegerToString(r,0,32));
         for(int c=0;c<6;c++)
   ...
   ObjectDelete(0,"vpnl_empty");
   ObjectDelete(0,"vpnl_columns"); // remove the former single-string header
   string headerText[6]={"TYPE","LOTS","TRIGGER","SL","TP","DISTANCE"};
   for(int c=0;c<6;c++)
   {
      int headerX=(c==0)?colX[c]:colCenter[c];
      ENUM_ANCHOR_POINT headerAnchor=(c==0)?ANCHOR_LEFT_UPPER:ANCHOR_UPPER;
      VirtualPanelSetLabel("vpnl_head"+IntegerToString(c,0,32),
                           headerX,y,headerText[c],textColor,fontSize,
                           "Consolas",headerAnchor);
   }

   // The natural boundaries of the six allocated column areas are colX[1..5].
   // Draw separators from the header row through the final visible order row.
   int separatorTop=y-MathMax(1,rowH/5);
   int separatorHeight=rowH*(shown+1)+MathMax(2,rowH/3);
   for(int c=0;c<5;c++)
      VirtualPanelSetVerticalLine("vpnl_vline"+IntegerToString(c,0,32),
                                  colX[c+1]-MathMax(1,(int)MathRound(2.0*scale)),
                                  separatorTop,separatorHeight,textColor);

   y+=rowH;

   int rowsToProcess=MathMax(shown,g_vPanelRowsRendered);
   for(int r=0;r<rowsToProcess;r++)
   {
      string rowName="vpnl_row"+IntegerToString(r,0,32);
      string markName="vpnl_mark"+IntegerToString(r,0,32);
      if(r>=shown)
      {
      ...
      if(UseVirtualStopOrders)
      {
         orderType=g_vOrders[r].type;
         orderSymbol=g_vOrders[r].symbol;
         orderLots=g_vOrders[r].lots;
         orderPrice=g_vOrders[r].price;
         orderSL=g_vOrders[r].sl;
         orderTP=g_vOrders[r].tp;
      }
      else
      {
         orderType=serverRows[r].type;
         orderSymbol=serverRows[r].symbol;
         orderLots=serverRows[r].lots;
         orderPrice=serverRows[r].price;
         orderSL=serverRows[r].sl;
         orderTP=serverRows[r].tp;
      }

      // BUY rows use the same dark color as the panel title. SELL rows remain
      // red, using the darker configured Sell Stop color. The marker, order
      // type and every value in the row share that same row color.
      color rowColor=(orderType==OP_SELLSTOP)?VirtualSellPanelColor:textColor;
      string typeText=(orderType==OP_BUYSTOP)?"BUY STOP":"SELL STOP";
      int digits=(int)SymbolInfoInteger(orderSymbol,SYMBOL_DIGITS);
      double point=SymbolInfoDouble(orderSymbol,SYMBOL_POINT);
      double market=(orderType==OP_BUYSTOP)
                    ?SymbolInfoDouble(orderSymbol,SYMBOL_ASK)
                    :SymbolInfoDouble(orderSymbol,SYMBOL_BID);
      int distance=(point>0.0)?(int)MathRound(MathAbs(orderPrice-market)/point):0;

      string slText=(orderSL>0.0)?DoubleToString(orderSL,digits):"-";
      string tpText=(orderTP>0.0)?DoubleToString(orderTP,digits):"-";
      string distanceText=IntegerToString(distance)+" pt";
      string cellText[6];
      cellText[0]=typeText;
      cellText[1]=DoubleToString(orderLots,2);
      cellText[2]=DoubleToString(orderPrice,digits);
      cellText[3]=slText;
      cellText[4]=tpText;
      cellText[5]=distanceText;

      int rowY=y+r*rowH;

      // Use a real text glyph instead of a rectangle. It shares the exact Y
      // coordinate and font metrics of BUY/SELL STOP, so it cannot sit above
      // or below the order text.
      if(ObjectFind(0,markName)>=0 &&
         (ENUM_OBJECT)ObjectGetInteger(0,markName,OBJPROP_TYPE)!=OBJ_LABEL)
         ObjectDelete(0,markName);
      VirtualPanelSetLabel(markName,textX,rowY,"|",rowColor,fontSize,
                           "Consolas",ANCHOR_LEFT_UPPER);

      // Remove the old combined row label, then draw each cell independently.
      // Numeric values remain centred, using a normal-weight font.
      ObjectDelete(0,rowName);
      for(int c=0;c<6;c++)
      {
         string cellName="vpnl_cell"+IntegerToString(r,0,32)+"_"+IntegerToString(c,0,32);
         int cellX=(c==0)?colX[c]:colCenter[c];
         ENUM_ANCHOR_POINT cellAnchor=(c==0)?ANCHOR_LEFT_UPPER:ANCHOR_UPPER;
         string cellFont="Consolas";
         VirtualPanelSetLabel(cellName,cellX,rowY,cellText[c],rowColor,fontSize,
                              cellFont,cellAnchor);
      }
   }

   g_vPanelRowsRendered=shown;
   ObjectDelete(0,"vpnl_more");
}

void VirtualPanelCleanup()
{
   VirtualPanelDeleteObjects();
   for(int i=0;i<g_vCount;i++) VRemoveLine(g_vOrders[i].ticket);
}

// Fast retry interval for triggered virtual orders. A millisecond timer also
// calls the processor, so a rejected/disconnected request does not need to wait
// for the next market tick before being attempted again.
#define VIRTUAL_FAST_RETRY_MS       50
#define VIRTUAL_SLOW_RETRY_MS     1000

ulong VirtualRetryDelayMs(uint retcode)
{
   switch(retcode)
   {
      // The cases explicitly requested: rejected orders and connection loss,
      // plus other short-lived price/server errors, are retried very quickly.
      ...
      case TRADE_RETCODE_PRICE_OFF:
         return VIRTUAL_FAST_RETRY_MS;

      // Back off slightly when the server says requests are too frequent or
      // temporarily locked, otherwise a 50 ms loop could prolong throttling.
      case TRADE_RETCODE_TOO_MANY_REQUESTS:
      case TRADE_RETCODE_LOCKED:
         return 250;
   }
   return VIRTUAL_SLOW_RETRY_MS;
}

bool VirtualRetcodeIsSuccess(uint retcode)
{
...
void VirtualScheduleRetry(int idx,uint retcode)
{
   if(idx<0 || idx>=g_vCount) return;
   g_vOrders[idx].sendPending=false;
   g_vOrders[idx].requestId=0;
   g_vOrders[idx].requestStartUs=0;
   g_vOrders[idx].nextRetryMs=GetTickCount64()+VirtualRetryDelayMs(retcode);
}

// Send the triggered virtual stop as a raw asynchronous MQL5 market request.
// This is used ONLY for virtual stop activation. OrderSendAsync returns as soon
// as the request has been accepted for transmission; the final server result is
// matched safely by request_id in OnTradeTransaction, preventing duplicate sends.
bool VirtualSendMarketFast(int idx)
{
   if(idx<0 || idx>=g_vCount) return false;
   if(!UseVirtualStopOrders || !g_vOrders[idx].triggered || g_vOrders[idx].sendPending)
      return false;

   ulong nowMs=GetTickCount64();
   if(nowMs<g_vOrders[idx].nextRetryMs) return false;

   // During a server/terminal disconnect, keep the triggered order armed and
   // retry automatically every 50 ms. No request is discarded.
   if(!TerminalInfoInteger(TERMINAL_CONNECTED))
   {
      g_vOrders[idx].nextRetryMs=nowMs+VIRTUAL_FAST_RETRY_MS;
      return false;
   }
   ...
   {
      g_vOrders[idx].nextRetryMs=nowMs+VIRTUAL_SLOW_RETRY_MS;
      return false;
   }

   MqlTick tick;
   ZeroMemory(tick);
   if(!SymbolInfoTick(g_vOrders[idx].symbol,tick) || tick.bid<=0.0 || tick.ask<=0.0)
   {
      g_vOrders[idx].nextRetryMs=nowMs+VIRTUAL_FAST_RETRY_MS;
      return false;
   }
   ...
   bool isBuy=(g_vOrders[idx].type==OP_BUYSTOP);
   request.action=TRADE_ACTION_DEAL;
   request.symbol=g_vOrders[idx].symbol;
   request.volume=g_vOrders[idx].lots;
   request.type=isBuy?ORDER_TYPE_BUY:ORDER_TYPE_SELL;
   request.price=isBuy?tick.ask:tick.bid;
   request.sl=g_vOrders[idx].sl;
   request.tp=g_vOrders[idx].tp;
   request.deviation=50;
   request.magic=(ulong)g_vOrders[idx].magic;
   request.comment=g_vOrders[idx].comment;
   request.type_filling=MT4SelectFilling(g_vOrders[idx].symbol);

   g_vOrders[idx].retryCount++;
   ulong requestStartUs=GetMicrosecondCount();
   bool accepted=::OrderSendAsync(request,result);
   if(!accepted && result.retcode==0) result.retcode=TRADE_RETCODE_ERROR;
   g_mt4_lastError=TradeRetcodeToMT4Error(result.retcode);

   if(accepted && VirtualRetcodeIsSuccess(result.retcode))
   {
      // Normal async path: wait for the final server response carrying the same
      // request_id. While pending, OnTick/OnTimer cannot submit a duplicate.
      if(result.request_id>0)
      {
         g_vOrders[idx].sendPending=true;
         g_vOrders[idx].requestId=result.request_id;
         g_vOrders[idx].requestStartUs=requestStartUs;
         g_vOrders[idx].nextRetryMs=0;
         return true;
      }

      // Defensive fallback for environments that return a final ticket without
      // a request_id (for example, some tester/build combinations).
      long realTicket=(long)((result.order>0)?result.order:result.deal);
      PrintFormat("[VIRTUAL-FAST] virtual stop order #%I64d triggered -> real market order #%I64d accepted",
                  g_vOrders[idx].ticket,realTicket);
      g_mt4_lastError=0;
      VirtualOrderRemoveAt(idx);
      return true;
   }

   // Requotes, explicit rejection, connection loss, timeout, price changes and
   // temporary server throttling retry in 50 ms. Fatal configuration errors are
   // retained and retried slowly instead of deleting the virtual order.
   if(g_vOrders[idx].retryCount==1 || (g_vOrders[idx].retryCount%20)==0)
      MT4PrintTradeReject("virtual fast open",request,result);
   VirtualScheduleRetry(idx,result.retcode);
   return false;
}

// Check current prices, latch a virtual trigger permanently, and submit/retry
// the real market order. Called from both OnTick and the 50 ms OnTimer event.
void VirtualOrdersProcessTick()
{
   if(!UseVirtualStopOrders) return;

   for(int i=g_vCount-1;i>=0;i--)
   {
      string sym=g_vOrders[i].symbol;
      if(g_vOrders[i].expiration>0 && TimeCurrent()>=g_vOrders[i].expiration)
      {
         PrintFormat("[VIRTUAL] virtual stop order #%I64d expired and was cancelled",g_vOrders[i].ticket);
         VirtualOrderRemoveAt(i);
         continue;
      }

      if(!g_vOrders[i].triggered)
      {
         MqlTick tick;
         ZeroMemory(tick);
         if(!SymbolInfoTick(sym,tick) || tick.bid<=0.0 || tick.ask<=0.0) continue;

         // Optional adaptive early trigger. When disabled, earlyDistance is
         // zero and the Virtual order triggers only at its original Entry.
         // The stored Entry, SL and TP are never modified.
         double earlyDistance=VirtualAdaptiveEarlyDistance(sym,tick,g_vOrders[i].type);

         bool trig=false;
         if(g_vOrders[i].type==OP_BUYSTOP  && tick.ask>=g_vOrders[i].price-earlyDistance) trig=true;
         if(g_vOrders[i].type==OP_SELLSTOP && tick.bid<=g_vOrders[i].price+earlyDistance) trig=true;
         if(!trig) continue;

         // Once touched, the order remains triggered even if price moves back
         // before a rejected/disconnected request can be resent.
         g_vOrders[i].triggered=true;
         g_vOrders[i].nextRetryMs=0;
      }

      if(!g_vOrders[i].sendPending)
         VirtualSendMarketFast(i);
   }

   // Keep the captions aligned with their price lines while the chart auto
   // scales, scrolls or is resized. This is lightweight and also runs from the
   // existing 50 ms Virtual timer.
   VUpdateAllLabelPositions();
}

// Final reply for OrderSendAsync. request_id gives an exact one-to-one match,
// so retrying after a rejection/connection error cannot duplicate another order.
void OnTradeTransaction(const MqlTradeTransaction &trans,
                        const MqlTradeRequest &request,
                        const MqlTradeResult &result)
{
   // Refresh immediately when a real server pending order is created, changed
   // or removed; do not wait for the next price tick to update the panel.
   VirtualPanelUpdate();

   if(!UseVirtualStopOrders || trans.type!=TRADE_TRANSACTION_REQUEST || result.request_id==0)
      return;

   int idx=VFindIndexByRequestId(result.request_id);
   if(idx<0) return;

   g_mt4_lastError=TradeRetcodeToMT4Error(result.retcode);

   // Measure only a final successful async response. This is the actual elapsed
   // time from local submission until the trade server confirms the request.
   if(VirtualRetcodeIsSuccess(result.retcode) && g_vOrders[idx].requestStartUs>0)
   {
      ulong nowUs=GetMicrosecondCount();
      if(nowUs>=g_vOrders[idx].requestStartUs)
      {
         double latencyMs=(double)(nowUs-g_vOrders[idx].requestStartUs)/1000.0;
         VirtualObserveServerLatency(g_vOrders[idx].symbol,latencyMs);
      }
   }

   if(VirtualRetcodeIsSuccess(result.retcode))
   {
      long virtualTicket=g_vOrders[idx].ticket;
      long realTicket=(long)((result.order>0)?result.order:result.deal);
      PrintFormat("[VIRTUAL-FAST] virtual stop order #%I64d triggered -> real market order #%I64d confirmed",
                  virtualTicket,realTicket);
      g_mt4_lastError=0;
      VirtualOrderRemoveAt(idx);
      return;
   }
   ...
   if(g_vOrders[idx].retryCount==1 || (g_vOrders[idx].retryCount%20)==0)
      MT4PrintTradeReject("virtual async open",request,result);
   VirtualScheduleRetry(idx,result.retcode);
}
...
{
   // ----- Virtual Stop Orders: intercept Buy Stop/Sell Stop before broker submission -----
   if(UseVirtualStopOrders && (cmd==OP_BUYSTOP || cmd==OP_SELLSTOP))
   {
      long vt=VirtualOrderAdd(symbol,cmd,volume,price,stoploss,takeprofit,comment,magic,expiration);
      g_mt4_lastTicket=vt;
      g_mt4_lastError=0;
      return vt;
   }
...
{
   // ----- Virtual Stop Orders -----
   if(VFindIndexByTicket(ticket)>=0)
   {
      bool vok=VirtualOrderModifyByTicket(ticket,price,stoploss,takeprofit,expiration);
      if(vok) PrintFormat("[VIRTUAL] modify #%I64d price:%.5f sl:%.5f tp:%.5f ok",ticket,price,stoploss,takeprofit);
      g_mt4_lastError=vok?0:4108;
      return vok;
   }
...
bool OrderDelete(long ticket,color arrow_color=clrNONE)
{
   ...
   if(VFindIndexByTicket(ticket)>=0)
   {
      bool vok=VirtualOrderDeleteByTicket(ticket);
      if(vok) PrintFormat("[VIRTUAL] delete #%I64d ok",ticket);
      g_mt4_lastError=vok?0:4108;
      return vok;
   }
...
int MT4OrdersTotal()
{
   return PositionsTotal()+OrdersTotal()+g_vCount;
}
...
bool OrderSelect(int index_or_ticket,int select,int pool=MODE_TRADES)
{
   if(select==SELECT_BY_TICKET)
   {
      ...
      int vidx=VFindIndexByTicket(ticket);
      if(vidx>=0)
      {
         g_selOrder.ticket=g_vOrders[vidx].ticket;
         g_selOrder.symbol=g_vOrders[vidx].symbol;
         g_selOrder.type=g_vOrders[vidx].type;
         g_selOrder.lots=g_vOrders[vidx].lots;
         g_selOrder.openPrice=g_vOrders[vidx].price;
         g_selOrder.closePrice=0.0;
         g_selOrder.sl=g_vOrders[vidx].sl;
         g_selOrder.tp=g_vOrders[vidx].tp;
         g_selOrder.openTime=g_vOrders[vidx].openTime;
         g_selOrder.closeTime=0;
         g_selOrder.expiration=g_vOrders[vidx].expiration;
         g_selOrder.profit=0.0;
         g_selOrder.swap=0.0;
         g_selOrder.commission=0.0;
         g_selOrder.comment=g_vOrders[vidx].comment;
         g_selOrder.magic=g_vOrders[vidx].magic;
         return true;
      }
   ...
   // ----- Virtual Stop Orders: listed after real pending orders in MODE_TRADES -----
   int vIdx=ordIdx-ordTotal;
   if(vIdx>=0 && vIdx<g_vCount)
   {
      g_selOrder.ticket=g_vOrders[vIdx].ticket;
      g_selOrder.symbol=g_vOrders[vIdx].symbol;
      g_selOrder.type=g_vOrders[vIdx].type;
...
input bool ShowVirtualOrdersPanel=true  ;    //Show/hide the auxiliary Virtual/Server Stop Orders panel
...
input string VirtualStopSettings="----------------------- Virtual Stop Orders -----------------------"  ;
input bool   UseVirtualStopOrders=true  ;    //Keep Buy/Sell Stop orders virtual instead of sending real pending orders to the broker
input bool   AdaptiveVirtualEarlyTrigger=true  ;    //Automatically calculate an early Virtual trigger from recent tick speed; false = trigger exactly at Entry
input color  VirtualBuyLineColor=clrDodgerBlue  ;    //Virtual Buy Stop line color
input color  VirtualSellLineColor=clrOrangeRed  ;    //Virtual Sell Stop line color
input color  VirtualBuyPanelColor=C'0,95,235'  ;    //Slightly stronger Buy Stop text/color on the virtual orders panel
input color  VirtualSellPanelColor=C'170,0,0'  ;    //Dark red Sell Stop text/color on the virtual orders panel
...
//BuildNFPDatesFromCalendar <<==--------   --------
 ...
 int OnInit()
 {
 VirtualPanelUpdate(); // initialize and clean the Virtual Stop Orders panel
 if(UseVirtualStopOrders)
    EventSetMillisecondTimer(VIRTUAL_FAST_RETRY_MS); // fast trigger/retry loop only in Virtual mode
...
//----- -----
 ...
 // Keep the 5-tick motion history warm even before a virtual pending order
 // is created. VirtualOrdersProcessTick reuses the same broker tick safely.
 if(UseVirtualStopOrders)
    VirtualTickMotionFeedSymbol(Symbol());
 VirtualOrdersProcessTick();
 VirtualPanelUpdate();
...
//----- -----
 ...
 {
   ObjectSetString(0,"line1",OBJPROP_TEXT,"The Gold Reaper V4.6 - OneChartSetup"); 
 }
